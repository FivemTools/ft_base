...
function Menu(data)
  ...
  self.ShowBigTitle = function()
    ...
    if self.title ~= nil then
      Text(self.title, 1, 1, self.x, self.y, self.titleScale, 255, 255, 255, 255)
    end
  ...
  self.SowHeader = function(selectedButton)
    ...
    Text(self.menuTitle, 0, 0, self.x - self.width / 2 + 0.005, y - self.height / 2 + 0.0028, 0.4, 255, 255, 255, 255)

    -- Numbers
    Text(countText, 0, 0, self.x + self.width / 2 - 0.0385, y - 0.015, 0.4, 255, 255, 255, 255)
  ...
  self.DrawMenuButton = function(data, x, y, width, height, selected)
    ...
    Text(data.text, 0, 0, x - self.width / 2 + 0.005, y - self.height / 2 + 0.0035, textScale, color.text.red, color.text.blue, color.text.green, 255)
    DrawRect(x, y, width, height, color.rect.red, color.rect.blue, color.rect.green, color.rect.alpha)

    if data.subText ~= nil then
      Text(data.subText, 0, true, x + self.width / 2 - 0.035, y - self.height / 2 + 0.0035, subTextScale, color.text.red, color.text.blue, color.text.green, 255)
    end
