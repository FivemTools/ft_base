...
function GetPlayerFromIdentifier(identifier)
  ...
  local player = nil
  local data = MySQL.Sync.fetchAll("SELECT * FROM " .. Database.players .. " WHERE identifier = @identifier", { ['@identifier'] = identifier })
...
function GetPlayerFromId(id)
  ...
  local player = nil
  local data = MySQL.Sync.fetchAll("SELECT * FROM " .. Database.players .. " WHERE id = @id", { ['@id'] = id })
...
function CreatePlayer(identifier)
  ...
  local result = MySQL.Sync.execute("INSERT INTO " .. Database.players .. " (`identifier`, `created_at`) VALUES (@identifier, @created_at)", { ['@identifier'] = identifier, ['@created_at'] = date } )
  return result
...
-- CellPlayer
RegisterServerEvent("ft_base:PlayerCall")
AddEventHandler('ft_base:PlayerCall', PlayerCall)
...
-- Update local Player
RegisterServerEvent("ft_base:SetLocalPlayer")
